...
import ga
def lppl_fit(ticker,start_date, end_date, i_date, j_date, k_date):
    ...
    # Get the data for the tikcer
    complete_data = yf.download(ticker,start_date, end_date)
    ...
    print("Calculating with the Downhill Simplex...")
    ...
    ga_total_params = ga.ga(ga_lower_bound, ga_upper_bound, ga_cost_func, epochs=30, num_pop=200, num_children=100, num_mutations=100)
    ...
    ga_pso_params = pso.optimize(opt_params, upper_bound, lower_bound, initial_value=ga_params, num_ind=300,num_neighbors=299, epochs=30)
    ...
    plt.legend(['Time Series: ' + str(np.round(exp_loss,4)), 'Downhill Simplex: '+str(np.round(downhill_loss,4)), 'GA: '+str(np.round(ga_loss,4)), 'PSO: '+str(np.round(pso_loss, 4)), 'GA-PSO: '+str(np.round(ga_pso_loss, 4)), 'PSO-GA: '+str(np.round(pso_ga_loss,4)), 'Combined'])
